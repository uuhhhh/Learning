...
public partial class ImpactParticles : CpuParticles2D {
	...
	public PropertyTweener EmitParticles(double intensity = DefaultIntensity) {
		...
		return _fadeTween.TweenProperty(this, nameof(Color).ToLower(), GetColorWithOpacity(0), Lifetime);
	}
	...
	private void SetEmissionIntensity(double intensity) {
		...
		InitialVelocityMin = Mathf.RoundToInt(Mathf.Lerp(0, _originalInitialVelocityMin, intensity));
		InitialVelocityMax = Mathf.RoundToInt(Mathf.Lerp(0, _originalInitialVelocityMax, intensity));
	}
}
